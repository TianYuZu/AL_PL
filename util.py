...
def get_loss_fn():
    loss_fn = AMSoftmax(AM_S, AM_M, is_amp=False)
    return loss_fn

# 读取PEDCC权重
def read_pkl():
    f = open(r'./100_512New.pkl','rb')
    a = pickle.load(f)
    f.close()
    return a
    ...
    def __init__(self, in_features, out_features, is_pedcc):
        ...
        if is_pedcc:
            ...
            for label_index in range(self.out_features):
                tensor_empty = torch.cat((tensor_empty, torch.from_numpy(map_dict[label_index]).float().cuda()), 0)
