...
svhn_train = SVHN('../svhn', split='train', download=True, transform=train_transform)
svhn_unlabeled   = SVHN('../svhn', split='train', download=True, transform=test_transform)
svhn_test  = SVHN('../svhn', split='test', download=True, transform=test_transform)
...
def train(model, loss_fn,loss_fn2, optimizer, scheduler, dataloaders, num_epochs, epoch_loss):
    ...
    best_acc = 0.
    checkpoint_dir = os.path.join('./svhn', 'train', 'weights')
...
def get_uncertainty(model, unlabeled_loader):
    ...
    with torch.no_grad():
        for (inputs, labels) in unlabeled_loader:
            ...
            # map_dict = read_pkl()
            # NormcosDis=get_max_cosdis(fea_norm,map_dict)
...
if __name__ == '__main__':
    ...
    f=open(os.path.join('./log/newpedcc','logweibiaoqian_2.txt'),'w')
    ...
    for trial in range(TRIALS):
        ...
        train_loader = DataLoader(svhn_train, batch_size=BATCH,
                                  sampler=SubsetRandomSampler(labeled_set), 
                                  pin_memory=True)
        test_loader  = DataLoader(svhn_test, batch_size=BATCH)
        dataloaders  = {'train': train_loader, 'test': test_loader}
        
        # Model
        model= resnet.ResNetx18(num_classes=10)
        ...
        for cycle in range(CYCLES):
            ...
            unlabeled_loader = DataLoader(svhn_unlabeled, batch_size=BATCH,
            ...
            pseudo_loader = DataLoader(svhn_unlabeled, batch_size=BATCH,
                                          sampler=SubsetSequentialSampler(pseudo_set), # more convenient if we maintain the order of subset
                                          pin_memory=True)
            ...
            for idx in range(len(pseudo_set)):
                ...
                dataset1=svhn_train_copy.labels
                dataset2=svhn_train.labels

                svhn_train_copy.labels[pseudo_set[idx]]=pseudo_labels[idx]
            # labeled_set += list(torch.tensor(subset)[arg][:ADDENDUM].numpy())
            # unlabeled_set = list(torch.tensor(subset)[arg][ADDENDUM:].numpy()) + unlabeled_set[SUBSET:]

            # Create a new dataloader for the updated labeled dataset
            dataloaders['train'] = DataLoader(svhn_train, batch_size=BATCH,
                                              sampler=SubsetRandomSampler(labeled_set), 
                                              pin_memory=True)
        ...
        torch.save({
                ...
                './svhn/train/weights/active_resnet18_svhn_trial{}.pth'.format(trial))
